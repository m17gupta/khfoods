...
export const ProductForm = ({
  ...
  const defaultVariant =
    filledVariants?.find((variant) => variant.stock > 0) ?? filledVariants?.[0];
  ...
  const handleCart = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (quantity <= maxQuantity - (cartItem?.quantity ?? 0)) {
      setOverStock(false);
      if (quantity === 0) {
        alert("Please add quantity more than zero");
      } else {
        updateCart([
          {
            id: product.id,
            ...
            <fieldset aria-label={t("choose-size")} className="mt-2">
              ...
              >
                {product.sizes?.map((size) => {
                  const matchingVariant = findAvailableSizeVariant(
                    size.id ?? ""
                  );
